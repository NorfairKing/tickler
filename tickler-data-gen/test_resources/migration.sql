...
CREATE TABLE "intray_trigger"("id" INTEGER PRIMARY KEY,"user" BLOB NULL DEFAULT NULL,"identifier" BLOB NOT NULL,"url" VARCHAR NOT NULL,"username" VARCHAR NOT NULL,"access_key" BLOB NOT NULL,"added" TIMESTAMP NOT NULL,CONSTRAINT "unique_intray_trigger" UNIQUE ("identifier"));
CREATE TABLE "email_trigger"("id" INTEGER PRIMARY KEY,"user" BLOB NULL DEFAULT NULL,"identifier" BLOB NOT NULL,"address" VARCHAR NOT NULL,"verification_key" BLOB NOT NULL,"verified" BOOLEAN NOT NULL,"added" TIMESTAMP NOT NULL,CONSTRAINT "unique_email_trigger" UNIQUE ("identifier"));
