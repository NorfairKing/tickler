...
CREATE TABLE "intray_trigger"("id" INTEGER PRIMARY KEY,"user" BLOB NOT NULL,"identifier" BLOB NOT NULL,"url" VARCHAR NOT NULL,"username" VARCHAR NOT NULL,"access_key" BLOB NOT NULL,"added" TIMESTAMP NOT NULL,CONSTRAINT "unique_intray_trigger" UNIQUE ("identifier"));
CREATE TABLE "email_trigger"("id" INTEGER PRIMARY KEY,"user" BLOB NOT NULL,"identifier" BLOB NOT NULL,"address" VARCHAR NOT NULL,"verification_key" BLOB NOT NULL,"verified" BOOLEAN NOT NULL,"added" TIMESTAMP NOT NULL,CONSTRAINT "unique_email_trigger" UNIQUE ("identifier"));
CREATE TABLE "verification_email"("id" INTEGER PRIMARY KEY,"to" VARCHAR NOT NULL,"key" BLOB NOT NULL,"trigger" BLOB NOT NULL,"scheduled" TIMESTAMP NOT NULL,"email" INTEGER NULL REFERENCES "email" ON DELETE RESTRICT ON UPDATE RESTRICT);
CREATE TABLE "admin_notification_email"("id" INTEGER PRIMARY KEY,"contents" VARCHAR NOT NULL,"email" INTEGER NULL REFERENCES "email" ON DELETE RESTRICT ON UPDATE RESTRICT);
...
CREATE TABLE "triggered_email"("id" INTEGER PRIMARY KEY,"item" BLOB NOT NULL,"trigger" BLOB NOT NULL,"email" INTEGER NULL REFERENCES "email" ON DELETE RESTRICT ON UPDATE RESTRICT,"error" VARCHAR NULL,CONSTRAINT "unique_triggered_email" UNIQUE ("item","trigger"));
